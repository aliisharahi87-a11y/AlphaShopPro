...
async def coupon_start(update, context):
    ...
    # پاک کردن کد تخفیف قبلی هنگام شروع ورود کد جدید
    context.user_data.pop("coupon", None)

    await query.message.reply_text(
        tr(uid, "coupon_prompt"),
        reply_markup=menu(uid),
    )
...
async def coupon_input(update, context):
    ...
    # ذخیره کد برای خرید بعدی
    context.user_data["coupon"] = used

    # محاسبه میزان تخفیف برای نمایش
    if used["percent"]:
        discount_text = f"{used['percent']}%"
    else:
        discount_text = f"{used['amount']:,} تومان"
    ...
    if lang(uid) == "fa":
        message = (
            ...
            "این تخفیف در خرید بعدی شما اعمال می‌شود. 🌹\n"
            "لطفاً از منوی زیر سرویس موردنظر خود را انتخاب کنید."
        )
    else:
        message = (
            "🎉 <b>Coupon activated successfully!</b>\n\n"
