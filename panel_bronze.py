...
def _config():
    return (
        _clean_url(os.getenv("BRONZE_PANEL_URL") or os.getenv("PANEL_URL")),
        (os.getenv("BRONZE_PANEL_USERNAME") or os.getenv("PANEL_USERNAME") or "").strip(),
        (os.getenv("BRONZE_PANEL_PASSWORD") or os.getenv("PANEL_PASSWORD") or "").strip(),
    )
...
async def _login(session):
    ...
    if not panel_url:
        raise RuntimeError(" Bronze panel URL is not configured")
    if not username or not password:
        raise RuntimeError(" Bronze panel credentials are not configured")
    async with session.post(
        ...
        if response.status != 200:
            raise RuntimeError(f" Bronze login HTTP {response.status}: {data}")
        token = data.get("access_token") or data.get("token") if isinstance(data, dict) else None
        if not token:
            raise RuntimeError(f" Bronze login returned no token: {data}")
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        try:
            ...
            async with session.post(f"{panel_url}/api/user", headers=headers, json=payload) as response:
                data = await _json(response)
                print(f"🟡 BRONZE CREATE HTTP {response.status}: {data}")
            ...
            if not connection:
                ...
                for path in ([f"{panel_url}/api/user/{user_id}/subscription/links"] if user_id else []) + [f"{panel_url}/api/user/{username}/subscription/links"]:
                    try:
                        ...
                        pass
            if not connection:
                raise RuntimeError(f" Bronze user created but no connection link returned: {data}")
            ...
            print(f"❌ BRONZE EXCEPTION: {exc!r}")
