...
def _panel_config(service="gold"):
    ...
    if service == "bronze":
        return (
            _clean_panel_url(
                os.getenv("BRONZE_PANEL_URL", "")
            ),
            os.getenv("BRONZE_PANEL_USERNAME", "").strip(),
            os.getenv("BRONZE_PANEL_PASSWORD", "").strip(),
        )
...
def _fix_bronze_https(value, panel_url=""):
    """Build the complete Bronze subscription URL."""
    if not value:
        return ""
    ...
    # PasarGuard returns subscription_url like:
    # /sub/xxxxxxxx
    # It must use the Bronze panel hostname.
    if value.startswith("/"):
        base = _clean_panel_url(panel_url)
        if base:
            return base + value

    # Handle malformed https:/...
    if value.startswith("https:/"):
...
async def _pasargard_create_customer(
    ...
    async with aiohttp.ClientSession(timeout=timeout, connector=connector) as session:
        try:
            ...
            connection = _fix_bronze_https(_extract_connection(data, panel_url), panel_url)
            ...
            if not connection:
                ...
                connection = _fix_bronze_https(_extract_connection(user_data, panel_url), panel_url)
...
async def _pasarguard_user_info(username, service="bronze"):
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        try:
            ...
            connection = _fix_bronze_https(_extract_connection(user_data, panel_url), panel_url) or (links[0] if links else "")
            return {"ok": True, "user": user_data, "subscription_url": connection, "expire": user_data.get("expire"), "data_limit": user_data.get("data_limit"), "used_traffic": user_data.get("used_traffic"), "status": user_data.get("status")}
        except Exception as exc:
            ...
            for endpoint in endpoints:
                async with session.put(endpoint, headers=headers, json=payload) as r:
                    ...
                    if r.status in (200, 201):
                        ...
                        connection = _fix_bronze_https(_extract_connection(data, panel_url), panel_url) or _extract_connection(current, panel_url) or (links[0] if links else "")
...
async def get_customer_info(username):
    return await _pasarguard_user_info(username, service="bronze")
...
async def create_customer(username, gb, unlimited=False, days=30, group_ids=None, note=""):
    return await _pasargard_create_customer(
        ...
        group_ids=group_ids, note=note, service="bronze"
    )

async def extend_customer(username, days=30):
    return await _extend_pasargard_customer(username, days=days, service="bronze")
